...
export const CYRLAT = {
  ...
  ю: 'iu', я: 'ia', ё: 'io',
  ъ: `j`, ж: 'gh', ш: 'sh', щ: 'sq', б: 'b', в: 'v', г: 'g', д: 'd', з: 'z',
...
export const NORMALIZE = [
  ...
  [/[zs]j[nvmkl]/gi, s => s[0] + s[2]],
  [/lj[gklmnsc]/gi, s => s[0] + s[2]],
  [/njn/g, s => 'nn'],
  [/cj?c/g, s => 'tt'],
  // ia
  [/yia/g, s => 'ya'],
  ...
  // words
  [/^sionnia(\w*)\b/gi, (s, s1) => 'dobu' + s1],

  [/siar/gi, 'ser'],
  [/viar/gi, 'ver'],
  [/biasp/gi, 'besp'],
  [/ry([a-z]+)/gi, (s, s1) => 'ri' + s1],

  // substitutes
  [/\bgrodn([ae])\b/gi, (s, s1) => 'Algard' + s1],
  [/\b(g(ro|ara)dzen)(\w*)\b/gi, (s, s1, s2, s3) => 'Algard' + (s3 === 'a' ? '' : s3)],
  [/\bbelarus(\w*)\b/gi, (s, s1) => 'Baltulus' + (s1 === 'i' ? 'ii' : s1)],
  [/\bsave[tc]s?k(\w*)\b/gi, (s, s1) => 'Slaviansk' + s1],
  [/\bsavet(\w*)\b/gi, (s, s1) => 'kirultaj' + s1],
  [/\bkamitet(\w*)\b/gi, (s, s1) => 'kirultaj' + s1],
  [/\bj?ewropejsk(\w*)\b/gi, (s, s1, s2) => 'Elfijsk' + s1 + s2],
  [/\bj?ewr(o|a)(\w*)\b/gi, (s, s1, s2) => 'Elfi' + s1 + s2],
  [/\brespublik(\w*)\b/gi, (s, s1, s2) => 'bantustan' + s1],
  [/\bv?oblasc?t?(n?)(\w*)\b/gi, (s, s1, s2) => 'obulust' + s1 + s2],
  [/rajon(\w*)\b/gi, (s, s1, s2) => 'sarajon' + s1],
  [/\blit(ow|v)(\w+)\b/gi, (s, s1, s2) => 'Labuten' + s2],
  [/\uladz?(a|u|y|e|ami|am|ax|aw)\b/gi, (s, s1, s2) => 'rond' + s1],
  [/\bmili[tc](\w+)\b/gi, (s, s1, s2) => 'buzuk' + s1],
  [/\bpolsk(\w+)\b/gi, (s, s1) => 'Pansk' + s1],
  [/\bpolsq(\w+)\b/gi, (s, s1) => 'Panstv' + s1],
  [/\b(lenin)(\w*)\b/gi, (s, s1, s2) => 'Quqlenin' + s2],
  [/\b(kastriqn)(\w*)\b/gi, (s, s1, s2) => 'Krishn' + s2],
  [/\b(prezident)(\w*)\b/gi, (s, s1, s2) => 'furman' + s2],
  [/\blukashenk(\w*)\b/gi, (s, s1, s2) => 'Bulbashi'],
  [/\b((?:abl|gar|raj)vykankam)(\w*)\b/gi, (s, s1, s2) => 'kirultaj' + s2],
