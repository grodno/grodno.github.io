...
import { methodName, nextId, runInBrowser } from './utils.js';
...
export class Component {
  ...
  constructor(Ctor, options) {
    ...
    if (parent) {
      ...
      this.app.$resources = { ...props.resources, app: this.app };
      this.impl.$ = this;
    }
  }
  ...
  emit(key, data, callback) {
    ...
    const event = {
      data,
      callback: (error, r) => racer(() => callback ? callback(error, r) : this.up({ error, ...r })),
      ...data,
    };
    const ref = type === 'this' ? this.impl : this.app[type];
    try {
