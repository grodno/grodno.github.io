...
export function placeholder(expr) {
  ...
  const initGettr = (key[0] === ':')
    ? (fn => c => c.pipe(fn(c), 'R'))(stringInterpolation(key.slice(1).trim()))
    : c => c.prop(key)
...
export function compileIf({ tag, attrs, uid, nodes }) {
  ...
  if ((expr.slice(0, 2) === '<-')) {
    const [key, ...pipes] = expr.slice(2).split('|').map(s => s.trim());
    const pipec = withPipes(pipes);
    const applicator = neg ? (rr => ({ $data: !pipec(c, rr) })) : (rr => ({ $data: (!!pipec(c, rr)) }));
  ...
  attrs.forEach((v, k) => {
    if (k.slice(0, 3) !== 'ui:') {
      ...
      if (v2 === '<-') {
        ...
        const pipec = withPipes(pipes);
        (r.inits || (r.inits = [])).push(c => c.connect(expr(c), rr => ({ [k]: pipec(c, rr) })));
      } else if (v2 === '->') {
