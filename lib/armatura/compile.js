...
const hasSlot = (c, id) => {
  let r = false;
  if (id && id != 'default') {
    c.content.forEach((e) => { r = r || (e.tag === c.tag + ':' + id) });
  } else {
    c.content.forEach((e) => { r = r || (e.tag.slice(0, c.tag.length + 1) !== c.tag + ':') });
  }
  return r
}
...
export function compileTag({ attrs, uid, nodes }) {
  ...
  const r = {
    tag: 'ui:fragment',
    uid: 'tag:' + expr + uid,
    $tag: { attrs: filterMapKey(attrs, 'tag'), nodes: nodes.map(compileNode) },
  };
  if ((expr.slice(0, 2) === '<-')) {
