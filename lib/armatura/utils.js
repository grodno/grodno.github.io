...
export const DOM_SETTERS = {
    ...
    data: function (e, v) {
        if (e.$dataset) {
            Object.keys(e.$dataset).forEach(k => { if (!v || !(k in v)) { delete e.dataset[k] } });
        }
        e.$dataset = v || {};
        if (v) {
    ...
    click: function (e, v) {
        this.setAttribute('click:click', !v ? null : (ev) => {
            this.$attributes.click({ value: e.value, ...e.$dataset }, ev);
            return false;
        });
    },
    scroll: function (e, v) {
        this.setAttribute('scroll:scroll', !v ? null : (ev) => {
            this.$attributes.scroll({ value: e.value, ...e.$dataset }, ev);
            return false;
        });
    },
