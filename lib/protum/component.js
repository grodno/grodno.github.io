...
function resolveRegular(acc, owner, { tag, updates, inits, nodes, uid, id, ref, $if, $each }) {
  ...
  return acc.set($tag + uid, { tag: $tag, id, ref, owner, inits, $if, $each, props, content });
}
...
export class Component {
  ...
  renderFragment() {
    ...
    if (this.$each) {
      ...
      const { tag, updates, nodes, uid } = itemNode;
      ...
      if (data && data.length) {
        data.forEach((d, index) => {
          ...
          resolveTemplate(this.owner, { tag, updates, nodes, uid: id }, acc);
        });
      } else if (!data) {
