...
export const NORMALIZE_LETTERS = [
  ...
  [/aja\b/gi, function (s) { return 'ea'; }],
  [/[cln]j[cln]/g, function (s, g1, g2) { return s[0] + (s[0] === s[2] ? '' : s[1]) + s[2]; }],
  [/[yi]j[a]/gi, function (s) { return s[0] + s[2]; }],
  [/[ayiu]e[oue]/gi, function (s) { return s[0] + s[2]; }],
  [/[psgklfzcvbnm]j[a]/gi, function (s) { return s[0] + 'i' + s[2]; }],
...
export const NORMALIZE_WORDS = [
  ...
  [/\b(belarus)(\w*)\b/gi, function (s, s1, s2) { return 'Baltorus' + s2; }],
  [/\b(save[tc])s?k?(\w*)\b/gi, function (s, s1, s2) { return 'Slavian' + s2; }],
  [/\bj?ewr(o|a)(\w*)\b/gi, function (s, s1, s2) { return 'Euro' + s2; }],
  [/\respublik(\w*)\b/gi, function (s, s1, s2) { return 'Impery' + s1; }],
  [/\bv?oblasc?n?(\w*)\b/gi, function (s, s1, s2) { return 'zon' + s1; }],
  ...
  [/\b(lenin)(\w*)\b/gi, function (s, s1, s2) { return 'Babaj' + s2; }],
