...
Object.dom = ((_win) ->
    ...
    init: (->
        ...
        fn_camelCase = (all, letter) -> letter.toUpperCase()

        _getAllWidgets = (e, all=[]) ->
            return  e.querySelectorAll "[x-widget]" if _doc.querySelectorAll

            all.push (c for c in e.children if c.getAttribute("x-widget"))...
            (_getAllWidgets c, all) for c in e.children
            all
        ...
        applyWidgetInit = (v) ->
            ...
            for a in v.attributes when not (n=a.name).indexOf("x-")
                z =v.getAttribute(n)
                if z[0] is '@'
                    z = Object.parse(z[1..])
                meta[n[2..].replace(re_dashAlpha, fn_camelCase)] = z 
        ...
        (root, opts) ->
            ...
            onCreated= (err, rr) ->
                ctx = Object.update(handleError: handleError, parentEntity: rr, opts)
                (applyWidgetInit.call ctx, node) for node in _getAllWidgets(rr.domNode)

            return onCreated(null, root) if root
