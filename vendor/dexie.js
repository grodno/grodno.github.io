...
export default (function (global, factory) {
    ...
    function delByKeyPath(obj, keyPath) {
        if (typeof keyPath === 'string') { setByKeyPath(obj, keyPath, undefined); } else if ('length' in keyPath) {
    ...
    */
    ...
    function addPossiblyUnhandledError(promise) {
        ...
        if (!unhandledErrors.some(function (p) { return p._value === promise._value; })) { unhandledErrors.push(promise); }
    }
    ...
    function Dexie(dbName, options) {
        ...
        */
        ...
        this.verno = 0;
        this.open = function () {
            if (isBeingOpened || idbdb) { return dbReadyPromise.then(function () { return dbOpenError ? rejection(dbOpenError) : db; }); }
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (!hasDeleteHook) {
                    for (var i = 0; i < len; ++i) {
                        ...
                        req.onerror = eventRejectHandler(reject);
                        if (i === lastItem) { req.onsuccess = wrap(function () { return resolve(); }); }
                    }
                } else {
                ...
                between: function (lower, upper, includeLower, includeUpper) {
                    ...
                    try {
                        if ((cmp(lower, upper) > 0) ||
                            (cmp(lower, upper) === 0 && (includeLower || includeUpper) && !(includeLower && includeUpper))) { return emptyCollection(this); } // Workaround for idiotic W3C Specification that DataError must be thrown if lower > upper. The natural result would be to return an empty collection.
            ...
            if (filter) {
                req.onsuccess = trycatcher(function filter_record() {
                    ...
                    if (cursor) {
                        ...
                        if (filter(cursor, function (advancer) { c = advancer; }, resolve, reject)) { wrappedFn(cursor.value, cursor, function (advancer) { c = advancer; }); }
                        c();
                    } else {
                        resolve();
                    }
