...
*/
hljs.highlightBlock = function(block, tabReplace, useBR){
  /*
  ...
  */
  ...
  function mergeStreams(stream1, stream2, value) {
    ...
    function open(node) {
      ...
      for (var i = 0; i < node.attributes.length; i++) {
        ...
        if (attribute.value !== undefined && attribute.value !== false && attribute.value !== null) {
          result += '="' + hljs.escape(attribute.value) + '"';
        }
      }
      return result + '>';
    }
    ...
    while (stream1.length || stream2.length) {
      ...
      result += hljs.escape(value.substr(processed, current.offset - processed));
    ...
    return result + hljs.escape(value.substr(processed));
  }
  ...
  if (result.second) {
    block.second = {
      language: result.second.language,
      kw: result.second.keyword_count,
      re: result.second.relevance
    };
  }
}
...
hljs.LANGUAGES.xml = hljs.LANGUAGES.html = function(){
